...
function createNewEnv(receiveTheOriginalEnv=null) {
    envList.push(null);
    (function(theElem) {
    ...
    if (receiveTheOriginalEnv) receiveTheOriginalEnv.res=envList[envList.length-1];
    return envList[envList.length-1].wrapper;
}

function onFinishInit(accel) {
    accel.id=-1;
    accel.type="master";
...
function onMessage(obj) {
    ...
    if (content.type==="pull") {
        ...
        workerList[obj.id].time = content.time;
...
function cleanup() {
    for (var i in workerList) {
        console.log("worker time: " + workerList[i].time);
