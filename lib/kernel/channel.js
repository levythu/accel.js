...
function Channel(bufferSize=0) {
    ...
    if (bufferSize==0) this.outBuffer=this.buffer;
    else this.outBuffer=new Queue();
...
Channel.prototype._tryToPushInBuffer=function() {
    while (this.outBuffer.GetLen()>0 && this.buffer.GetLen()<this.bufferSize) {
        var pusher=this.outBuffer.DeQueue();
        accel._msgRouter.Send(this, pusher.f, {
            r: 1,
        });
        pusher.hasResponded=true;
        this.buffer.EnQueue(pusher);
    }
}
    ...
    if ("ps" in content || "pl" in content) {
        if (conf.isMaster) {
            if ("ps" in content) {
                ...
                } else this.outBuffer.EnQueue(content);
            }
...
Channel.prototype.Dump=function(callback) {
    var that=this;
    this.Recv((res) => {
        if (callback(res)) {
            that.Dump(callback);
