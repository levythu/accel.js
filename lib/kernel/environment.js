...
DisEnv.prototype.syncWithPatch=function(patch) {
    for (i in patch) {
        ...
        if (patch[i].fvalue!=null) {
            ...
            this.actualData[i].value=that.funcEmitter(i);
            this.actualData[i].originFunc=eval(patch[i].fvalue);
        } else {
            this.actualData[i].version=patch[i].version;
            this.actualData[i].value=pacth[i].value
