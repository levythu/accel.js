        ...
        socket.on("message", function(message) {
            ...
            if (obj.type === "init") {
                if (obj.nodeType !== "remote") {
                    ...
                    createNewWorker(obj.nodeId, obj.masterEndPoint, obj.masterPort);
                }
            }
        });
    });
...
function createNewWorker(id, endpoint, port) {
    console.log("new worker: " + id);
    proc = childprocess.fork(path.join(__dirname, "../worker/localkernel/entry"), [endpoint, port], [0, 1, 2, "ipc"]);
}
